...
COMBINE_PERMUTATION_SCRIPT="$BIN_PATH/combine_permutations.py"  # <-- optional if you have it

combine_jobid_bootstrap=$(sbatch --parsable --dependency=afterok:$array_jobid_bootstrap --job-name=combine_bootstrap_outputs --wrap="\
module load miniconda; \
conda activate scmpra; \
...
combine_jobid_permutation=$(sbatch --parsable --dependency=afterok:$array_jobid_permutation --job-name=combine_permutation_outputs --wrap="\
module load miniconda; \
conda activate scmpra; \
...
empirical_pval_jobid=$(sbatch --dependency=afterok:$combine_jobid_bootstrap,$combine_jobid_permutation --job-name=empirical_pvals --wrap="\
module load miniconda; \
conda activate scmpra; \
