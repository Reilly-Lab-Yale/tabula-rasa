#!/usr/bin/env python
"""
Shendure obs NB ortho with current (phantom-enabled) codebase.
Barcode-level reporter -- no coarse reporter needed.
"""
...
OUT_ROOT = DATA_DIR
OUT_NAME = "shendure_obs_nb_phantom"
...
def main():
    ...
    with performance_report(filename=report_path):
        print("\nFitting by_cre...", flush=True)
        t_cre = time.time()
        ortho_obj.fit_by_cre_models(client=client, dat=shendure, nb_only=True)
        print(f"by_cre done in {time.time()-t_cre:.1f}s", flush=True)

        print("\nFitting by_cell_type...", flush=True)
        t_ct = time.time()
        ortho_obj.fit_by_cell_type_models(client=client, dat=shendure, nb_only=True)
