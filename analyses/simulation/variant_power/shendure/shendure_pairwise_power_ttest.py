#!/usr/bin/env python3
"""
Shendure pairwise CRE power analysis -- Welch t-test

Companion to shendure_pairwise_power_mwu.py. Reuses the sims already on
disk (produced by the MWU script) and runs Welch's t-test on the same
...
Usage:
    python shendure_pairwise_power_ttest.py [phase]
...
# ---------------------------------------------------------------------------
# Paths and parameters (mirror shendure_pairwise_power_mwu.py)
# ---------------------------------------------------------------------------
...
def _discover_sims():
    """Yield (cell_type, cohort_dir, sim_name) for every sim with MWU."""
    sims = []
    for ct in CELL_TYPES:
        ct_root = SIM_DIR / ct
        if not ct_root.exists():
            continue
        for cohort_dir in sorted(ct_root.glob("cohort_*")):
            if not cohort_dir.is_dir():
                continue
            for d in sorted(cohort_dir.iterdir()):
                if d.is_dir() and _sim_has_mwu(d):
                    sims.append((ct, cohort_dir, d.name))
    return sims
...
def phase_plot(client):
    sims = _discover_sims()
    sims = [
        (ct, cd, sn) for (ct, cd, sn) in sims if _sim_has_ttest(cd / sn)
    ]
    ...
    for ct, cohort_dir, sim_name in sims:
        sim = scm.de_novo_simulation(
            location=cohort_dir, name=sim_name, client=client
        )
        gt = sim.ground_truth
    ...
    power_df.to_parquet(
        OUTPUT_DIR / "shendure_pairwise_power_ttest_df.parquet", index=False
    )
    null_df.to_parquet(
        OUTPUT_DIR / "shendure_pairwise_null_ttest_df.parquet", index=False
    )
    ...
    fig.suptitle(
        "Welch t-test pairwise power -- Shendure\n"
        f"(alpha={ALPHA}, {N_LIBRARY_REPS}x{N_SIMS} reps per grid cell)",
        fontsize=13, y=1.01,
    )
    plt.tight_layout()
    svg_path = (
        OUTPUT_DIR / "shendure_pairwise_power_ttest_all_cell_types.svg"
    )
    ...
    svg_path2 = (
        OUTPUT_DIR / "shendure_pairwise_power_ttest_50pct_contours.svg"
    )
    ...
    def _make_slurm_client():
        cluster = SLURMCluster(
            ...
            job_extra_directives=[
                "-p priority",
                "--account=prio_skr2",
                "--job-name=shendure_pw_tt_worker",
                "--time=6:00:00",
                "--exclude=a1132u18n02",
