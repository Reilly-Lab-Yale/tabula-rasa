#!/usr/bin/env python3
"""
Shendure null calibration -- t-test

Measures per-anchor false positive rate of Welch's t-test under the null
(two independent CREs drawn at the same baseline mu) across the Shendure
(integrating, iPSC) dataset's activity range. No fold change offset, no
multiple testing correction -- raw p<0.05 rejection rate per anchor baseline.

Each cell type is simulated independently (Shendure has per-CT CRE sets).

Usage:
    python shendure_pairwise_null_calibration_ttest.py [phase]
...
SIM_DATE = "2026-04-10"
SIM_DIR = DATA_ROOT / "simulated" / f"{SIM_DATE}_shendure_null_ttest"
...
# Smallest CT must accommodate 2*24 + 1 = 49 CREs; assert at import time.
_min_n_cres = min(N_CRES_PER_CT.values())
assert _min_n_cres >= 2 * N_TOTAL_ANCHORS + 1, (
    f"Smallest CT has {_min_n_cres} CREs, need "
    f"{2 * N_TOTAL_ANCHORS + 1}"
)
...
def build_ground_truth(cell_type, n_cres, anchor_baselines, minP):
    names = []
    mus = []
...
def build_null_hypotheses(cell_type, n_anchors):
    rows = []
...
def phase_simulate(client):
    ...
    total_sims = len(CELL_TYPES) * N_LIBRARY_REPS
    ...
    for ct in CELL_TYPES:
        ...
        hs = build_null_hypotheses(ct, N_TOTAL_ANCHORS)
        cohort_dir = SIM_DIR / ct / "cohort_0"
        ...
        for lib_rep in range(n_remaining):
            gt_df = build_ground_truth(
                ct, n_cres, ANCHOR_BASELINES, MINP
            )
...
def phase_plot(client):
    sim_registry = []  # (ct, cohort_dir, sim_name)
    for ct in CELL_TYPES:
        cohort_dir = SIM_DIR / ct / "cohort_0"
        ...
        for d in sorted(cohort_dir.iterdir()):
            if d.is_dir() and _sim_is_complete(d):
                sim_registry.append((ct, cohort_dir, d.name))
    print(f"Loaded {len(sim_registry)} sims from disk.", flush=True)

    with open(OUTPUT_DIR / "shendure_null_ttest_sim_registry.json",
              "w") as f:
        json.dump(
            [(ct, str(p), n) for ct, p, n in sim_registry], f, indent=2
        )

    null_rows = []
    for ct, cd, sim_name in sim_registry:
        ...
        for i in range(sim.get_state_field("n_sims")):
            ...
            for _, row in m.iterrows():
                ...
                anchor_mu = gt[gt["cre_id"] == cre_name]["mu"].iloc[0]
                null_rows.append({
                    "cell_type": ct,
                    "anchor_idx": a_idx,
    ...
    null_df.to_parquet(
        OUTPUT_DIR / "shendure_null_ttest_df.parquet", index=False
    )
    ...
    fpr_grid.to_parquet(
        OUTPUT_DIR / "shendure_null_ttest_fpr_grid.parquet", index=False
    )
    ...
    # --- Heatmap: rows=cell_types, cols=anchors ---
    pivot = fpr_grid.pivot_table(
        index="cell_type", columns="anchor_idx", values="fpr"
    ).reindex(CELL_TYPES)

    fig, ax = plt.subplots(figsize=(14, max(3.5, 0.6 * len(CELL_TYPES) + 1)))
    ...
    sns.heatmap(
        ...
        yticklabels=[
            ("Pluripotent" if ct == "reference" else ct) for ct in pivot.index
        ],
    )
    ax.set_xlabel("Baseline activity (mu)", fontsize=10)
    ax.set_ylabel("Cell type", fontsize=10)
    ax.set_title(
        f"Shendure null FPR calibration -- Welch t-test (alpha={ALPHA})\n"
    ...
    plt.tight_layout()
    svg_path = OUTPUT_DIR / "shendure_null_ttest_fpr_heatmap.svg"
    ...
    for i, ct in enumerate(CELL_TYPES):
        ...
        label = "Pluripotent" if ct == "reference" else ct
    ...
    ax.set_title(
        f"Shendure null FPR calibration -- Welch t-test", fontsize=12,
    )
    ax.legend(fontsize=8, loc="upper right", ncol=2)
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    svg_path2 = OUTPUT_DIR / "shendure_null_ttest_fpr_lines.svg"
