#!/usr/bin/env python3
"""
Takeshi pairwise CRE power analysis -- Welch t-test

Measures Welch t-test power for pairwise CRE comparisons as a function of
(baseline activity, log2 fold change) in the Takeshi (piggyBac, oBC
barcode-level reporter) dataset.

Takeshi is integrating, so each cell type has its own fitted CRE set. Each
cell type is simulated independently with its own n_cres and
cells_per_cell_type. Cell types with fewer CREs are split into multiple
cohorts that tile the anchor space; results are aggregated across cohorts.

Single-condition (+reporter / obs): no -reporter/deflated comparison, since
this script is intended to characterize the power surface of the canonical
test configuration. The null_check rows embedded in hs_pairwise also report
FPR as a sanity check.

Usage:
    python takeshi_pairwise_power_ttest.py [phase]
...
def _cohorts_for_ct(n_cres):
    ...
    max_anchors = (n_cres - 1) // CRES_PER_ANCHOR
...
def _sim_is_complete(sim_dir):
    tt_dir = sim_dir / "tests" / "hs_pairwise" / "ttest"
...
def phase_simulate(client):
    ...
    for ct in CELL_TYPES:
        ...
        bound_ct.cells_per_cell_type = (
            scm.TAKESHI_BOUNDS.cells_per_cell_type.loc[[ct]]
        )
...
def phase_plot(client):
    ...
    with open(OUTPUT_DIR / "takeshi_pairwise_power_sim_registry.json",
              "w") as f:
        json.dump(
    ...
    for ct, cohort_idx, cohort_dir, sim_name in sim_registry:
        ...
        for i in range(sim.get_state_field("n_sims")):
            m = sim._merge_in_ground_truth(
                hypothesis_set_name="hs_pairwise", test_type="ttest", index=i
            )
    ...
    power_df.to_parquet(
        OUTPUT_DIR / "takeshi_pairwise_power_ttest_df.parquet", index=False
    )
    null_df.to_parquet(
        OUTPUT_DIR / "takeshi_pairwise_null_ttest_df.parquet", index=False
    )
    ...
    for ct in CELL_TYPES:
        ...
        if len(ct_null) > 0:
            ...
            print(f"  {ct}: null FPR@{ALPHA} = {fpr:.4f} (n={len(ct_null)})",
                  flush=True)
    ...
    fig.suptitle(
        "Welch t-test pairwise power -- Takeshi\n"
        f"(alpha={ALPHA}, {N_LIBRARY_REPS}x{N_SIMS} reps per grid cell)",
        fontsize=13, y=1.01,
    )
    plt.tight_layout()
    svg_path = OUTPUT_DIR / "takeshi_pairwise_power_ttest_all_cell_types.svg"
    ...
    ax.set_ylabel(f"Min log2(FC) for {int(power_threshold*100)}% power",
                  fontsize=11)
    ax.set_title(
        f"Minimum detectable fold change -- Takeshi "
        f"(Welch t-test, alpha={ALPHA}, {int(power_threshold*100)}% power)",
        fontsize=12,
    )
