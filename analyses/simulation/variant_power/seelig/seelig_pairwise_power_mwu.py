#!/usr/bin/env python3
"""
Seelig pairwise CRE power analysis -- MWU

Measures MWU test power for pairwise CRE comparisons as a function of
(baseline activity, log2 fold change) in the Seelig (non-integrating, no
transfection reporter) dataset.

Seelig has no reporter, so MWU is run with has_reporter=False -- the test
operates on the consider_missing-expanded count matrix (test results saved
under tests/<hs>/mwu_deflated/). Bounds are from SEELIG_BOUNDS (calibrated
against the MOIB NB fit; mu values are conditional on transfection).

Library structure mirrors cohen (episomal, shared library across cell types):
each cell type sees the same set of CREs, with one cohort sufficient because
the seelig library is large (1344 CREs). Two cell types: K562 and reference
(HepG2 -- the reference assignment per CLAUDE.md).

Usage:
    python seelig_pairwise_power_mwu.py [phase]

Phases:
    simulate  -- generate simulations and run MWU test (has_reporter=False)
...
# Seelig is non-integrating: same library across CTs. Per-CT fitted n_cres may
# vary slightly from the real library size; use the max so the simulated
# library matches the experiment.
...
def build_ground_truth(cell_types, anchor_indices, n_cres,
    ...
    Seelig is non-integrating: same CREs with same activities across cell
    types (mu is conditional-on-transfection per the MOIB calibration).
    """
    names = []
    mus = []
...
def _sim_is_complete(sim_dir):
    """A sim is complete if its mwu_deflated results dir exists and is non-empty."""
    mwu_dir = sim_dir / "tests" / "hs_pairwise" / "mwu_deflated"
    ...
    for cohort_idx, anchor_indices in enumerate(COHORTS):
        ...
        for lib_rep in range(n_remaining):
            ...
            sim = scm.de_novo_simulation(
                ...
                n_sims=N_SIMS,
                experiment_bounds=scm.SEELIG_BOUNDS,
                ground_truth=gt_df,
            )
            sim.gamut()
            sim.add_hypothesis_set("hs_pairwise", hs)
            # has_reporter=False: seelig has no transfection reporter, so the
            # test must operate on the CM-expanded matrix (phantom zeros from
            # un-detected (cell, CRE) pairs are real structural information).
            sim.mwu("hs_pairwise", has_reporter=False)
            sim.save()
...
def phase_plot(client):
    ...
    with open(OUTPUT_DIR / "seelig_pairwise_power_sim_registry.json", "w") as f:
        json.dump(
    ...
    power_df.to_parquet(OUTPUT_DIR / "seelig_pairwise_power_df.parquet",
                        index=False)
    null_df.to_parquet(OUTPUT_DIR / "seelig_pairwise_null_df.parquet",
                       index=False)
    ...
    fig.suptitle(
        "MWU pairwise power -- Seelig (no reporter, MOIB NB)\n"
        f"(alpha=0.05, {N_LIBRARY_REPS}x{N_SIMS} reps per grid cell)",
        fontsize=13, y=1.01,
    )
    plt.tight_layout()
    svg_path = OUTPUT_DIR / "seelig_pairwise_power_mwu_all_cell_types.svg"
    fig.savefig(svg_path, format="svg", bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {svg_path}", flush=True)

    # --- Per-CT panels (matches the cohen/shendure replot output) ---
    ...
    ax.set_ylabel(f"Min |log2 FC| for {int(power_threshold*100)}% power",
                  fontsize=11)
    ax.set_title(
        f"Minimum detectable fold change -- Seelig "
