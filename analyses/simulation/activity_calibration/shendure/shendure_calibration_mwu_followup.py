#!/usr/bin/env python3
"""
Shendure MWU null calibration follow-up.

Reuses the existing 2026-04-10_shendure_cal sim dirs (produced by
shendure_calibration_ttest_all_cell_types.py) and runs Mann-Whitney U on the
...
on shendure. Is the inflation t-test specific (catastrophic-cancellation /
...
Usage:
    python shendure_calibration_mwu_followup.py [phase]
...
CELL_TYPES = sorted(scm.SHENDURE_BOUNDS.cells_per_cell_type.index.tolist())
...
def phase_plot(client):
    ...
    for label, d in [("reporter", pvals_reporter), ("deflated", pvals_deflated)]:
        ...
        out_path = OUTPUT_DIR / f"shendure_null_pvals_mwu_{label}.parquet"
    ...
    summary_path = OUTPUT_DIR / "shendure_null_summary_mwu.parquet"
    ...
    fig.suptitle(
        "Shendure (piggyBac) -- MWU null calibration\n"
        "+reporter vs -reporter (deflated) -- cross-check vs t-test cal",
        fontsize=12, y=1.005,
    )
    plt.tight_layout()
    svg_path = OUTPUT_DIR / "shendure_calibration_mwu_reporter_comparison.svg"
    ...
    def _make_slurm_client():
        cluster = SLURMCluster(
            ...
            job_extra_directives=[
                "-p priority",
                "--account=prio_skr2",
                "--job-name=shendure_mwucal_worker",
                "--time=4:00:00",
