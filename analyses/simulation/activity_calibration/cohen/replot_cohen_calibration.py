"""Re-generate Cohen MWU calibration plots from saved sim data."""
...
def main():
    data_root = Path("/nfs/roberts/project/pi_skr2/shared/tabula_data_new/simulated")
    ...
    rows = []
    for sim_path in sorted(sim_dir.iterdir()):
        mwu_dir = sim_path / "tests" / "hs_all_ct" / "mwu"
        if not (mwu_dir.exists() and any(mwu_dir.iterdir())):
            continue
        sim = scm.de_novo_simulation(location=sim_dir, name=sim_path.name, client=client)
    ...
    for style_name, s in STYLES.items():
        ...
        for i, ct in enumerate(cell_types):
            ...
            # QQ plot (cap scatter at 50k points to keep SVG under GitHub's 100MB limit;
            # FPR / shape are unchanged for n>>50k).
            ax_q = axes[i, 1]
        ...
        fig.suptitle("MWU p-value calibration under null -- Cohen (episomal)",
