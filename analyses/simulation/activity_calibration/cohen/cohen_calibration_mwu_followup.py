#!/usr/bin/env python3
"""
Cohen MWU null calibration follow-up.

Reuses the existing 2026-04-10_cohen_cal sim dirs (produced by
cohen_calibration_ttest_all_cell_types.py) and runs Mann-Whitney U on the
same hs_all_ct hypothesis set, in both +reporter and deflated modes.

Cohen cal stores all sims directly under SIM_DIR (not per-cell-type
subdirs); each sim's ground truth contains all cell types simultaneously.
Per-CT split happens at result-collection time via
`comparison_cell_type` from `_merge_in_ground_truth`.

Motivation: cross-check whether t-test FPR inflation observed on takeshi
generalizes. MWU doesn't depend on normality.

Usage:
    python cohen_calibration_mwu_followup.py [phase]
        ...
        for d in sorted(SIM_DIR.iterdir()):
            ...
            sims.append(
                scm.de_novo_simulation(
                    location=SIM_DIR, name=d.name, client=client
                )
            )
    return sims
...
def phase_compute(client):
    ...
    sims = _discover_sims(client)
    print(f"Loaded {len(sims)} sims with ttest already done.", flush=True)

    n_to_run_rep = sum(1 for s in sims
                       if not _sim_has(s.location / s.name, "mwu"))
    print(f"mwu (+reporter): {n_to_run_rep} need running", flush=True)
    for sim in sims:
    ...
    n_to_run_def = sum(1 for s in sims
                       if not _sim_has(s.location / s.name, "mwu_deflated"))
    print(f"mwu (deflated): {n_to_run_def} need running", flush=True)
    for sim in sims:
...
def _collect_pvals_by_ct(sims, test_type):
    """Return {cell_type: np.array of p-values}, splitting by
    `comparison_cell_type` since cohen cal sims are flat (not per-CT)."""
    rows = []
    for sim in sims:
    ...
    if not rows:
        return {ct: np.array([]) for ct in CELL_TYPES}
...
def phase_plot(client):
    print(f"Loading sims for plot phase...", flush=True)
    sims = _discover_sims(client)
    sims = [s for s in sims
            if _sim_has(s.location / s.name, "mwu")]
    print(f"Loaded {len(sims)} sims with mwu done.", flush=True)

    pvals_reporter = _collect_pvals_by_ct(sims, "mwu")
    pvals_deflated = _collect_pvals_by_ct(sims, "mwu_deflated")

    for label, d in [("reporter", pvals_reporter), ("deflated", pvals_deflated)]:
        rows = []
        for ct, v in d.items():
            rows.append(pd.DataFrame({"cell_type": ct, "p_value": v}))
        out_path = OUTPUT_DIR / f"cohen_null_pvals_mwu_{label}.parquet"
    ...
    summary_path = OUTPUT_DIR / "cohen_null_summary_mwu.parquet"
    ...
    fig.suptitle(
        "Cohen (non-integrating, U6 reporter) -- MWU null calibration\n"
        "+reporter vs -reporter (deflated) -- cross-check vs t-test cal",
        fontsize=12, y=1.005,
    )
    plt.tight_layout()
    svg_path = OUTPUT_DIR / "cohen_calibration_mwu_reporter_comparison.svg"
