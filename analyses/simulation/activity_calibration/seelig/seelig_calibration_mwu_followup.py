#!/usr/bin/env python3
"""
Seelig MWU null calibration follow-up.

Reuses the existing 2026-04-10_seelig_cal sim dirs (produced by
seelig_calibration_ttest_all_cell_types.py) and runs Mann-Whitney U on the
same hs_all_ct hypothesis set, in the deflated condition only.

Seelig has no transfection reporter, so the +reporter condition is not
meaningful and is not run (matching the t-test cal script's deflated-only
scope). Each sim already has its data.parquet on disk and an hs_all_ct
hypothesis set; this script just adds a mwu_deflated/ result subdir
alongside the existing ttest_deflated/.

Motivation: cross-check whether t-test FPR inflation observed on takeshi
generalizes. MWU doesn't depend on normality.

Usage:
    python seelig_calibration_mwu_followup.py [phase]
...
SIM_DATE = "2026-04-10"
SIM_DIR = DATA_ROOT / "simulated" / f"{SIM_DATE}_seelig_cal"

CELL_TYPES = sorted(scm.SEELIG_BOUNDS.cells_per_cell_type.index.tolist())
    ...
    for cell_type in CELL_TYPES:
        ...
        if ct_dir.exists():
            for d in sorted(ct_dir.iterdir()):
                if not d.is_dir():
                    continue
                if not _sim_has(d, "ttest_deflated"):
                    continue
                sims_ct.append(
...
def phase_plot(client):
    ...
    for cell_type in CELL_TYPES:
        ...
        if ct_dir.exists():
            for d in sorted(ct_dir.iterdir()):
                if not d.is_dir():
                    continue
                if not _sim_has(d, "mwu_deflated"):
                    continue
                sims_ct.append(
                    scm.de_novo_simulation(
                        location=ct_dir, name=d.name, client=client
    ...
    rows = []
    for ct, v in pvals_deflated.items():
        rows.append(pd.DataFrame({"cell_type": ct, "p_value": v}))
    out_path = OUTPUT_DIR / "seelig_null_pvals_mwu_deflated.parquet"
    ...
    for ct, v in pvals_deflated.items():
        ...
        stats["cell_type"] = ct
        stats["condition"] = "deflated"
        summary_rows.append(stats)
    summary = pd.DataFrame(summary_rows)
    cols = ["cell_type", "condition", "n", "fpr", "fpr_lo", "fpr_hi",
            "ks_d", "ks_p"]
    summary = summary[cols]
    summary_path = OUTPUT_DIR / "seelig_null_summary_mwu.parquet"
    ...
    for i, ct in enumerate(CELL_TYPES):
        ...
        cells = scm.SEELIG_BOUNDS.cells_per_cell_type.get(ct, "?")
        v = pvals_deflated[ct]
        ...
        ax_h.set_title(f"{ct_display} -- -reporter (deflated)", fontsize=9)
        ...
        if n > 0:
            ...
            ax_q.scatter(qs, obs, s=2, alpha=0.7, color="coral")
        ax_q.plot([0, 1], [0, 1], "r--", lw=1)
        ax_q.set_title(f"{ct_display} -- deflated QQ", fontsize=9)
    ...
    fig.suptitle(
        "Seelig (no reporter) -- MWU null calibration\n"
        "deflated only -- cross-check vs t-test cal",
        fontsize=12, y=1.005,
    )
    plt.tight_layout()
    svg_path = OUTPUT_DIR / "seelig_calibration_mwu_deflated.svg"
