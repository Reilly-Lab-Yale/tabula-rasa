#!/usr/bin/env python3
"""
Seelig Welch t-test null calibration -- all cell types.

Mirrors `seelig_power_ttest_all_cell_types.py` exactly in scope and infra
(N_LIBRARY_REPS=156, N_SIMS=5, per-CT sims, single deflated condition).
...
Seelig has no transfection reporter, so the only realistic test condition is
the deflated (has_reporter=False) Welch t-test on observed nonzero counts.
There is no +reporter condition to compare against; MOIB-at-test-time was
...
Usage:
    python seelig_calibration_ttest_all_cell_types.py [phase]

Phases:
    simulate  -- generate simulations and run t-test (deflated only)
...
# n_cres per cell type comes from the canonical CM phantom ortho (same as
# the seelig power script).
...
def phase_simulate(client):
    ...
    # --- t-test: deflated (only condition) ---
...
def phase_plot(client):
    ...
    out_path = OUTPUT_DIR / "seelig_null_pvals_deflated.parquet"
    ...
    summary_path = OUTPUT_DIR / "seelig_null_summary.parquet"
    ...
    for i, ct in enumerate(CELL_TYPES):
        # Seelig: "reference" was chosen as the higher-cell-count of the two
        # cell lines (HepG2 5858 vs K562 4478). Both are cell lines, neither
        # is pluripotent.
        ...
        n = len(v)
        if n > 0:
            # Quantile-thinned QQ: evaluate at evenly-spaced quantiles instead
            # of plotting every point. Preserves QQ shape with O(N_q) points.
    ...
    fig.suptitle(
        "Seelig (no reporter) -- Welch t-test null calibration\n"
        "deflated test on nonzero counts",
        fontsize=12, y=1.005,
    )
    plt.tight_layout()
    svg_path = OUTPUT_DIR / "seelig_calibration_ttest_deflated.svg"
