#!/usr/bin/env python
"""
Shendure NB-only by-cell-type fit (consider_missing=True, CPU).
...
CPU-only: shendure+consider_missing design matrices are too large for
GPU VRAM.  Uses 16 workers × 96GB.  Expected wall time ~30-40h.
"""
import signal
...
name = "shendure_cm_nb_20260329_by_cell_type"
...
cluster = SLURMCluster(
    ...
    job_extra_directives=[
        ...
        "--job-name=shend_cm_nb_ct_w", "--time=2-23:40:00",
...
try:
    if (path / name).is_dir():
        ...
        primordial.fit_by_cell_type_models(client=client, dat=shendure, nb_only=True)
