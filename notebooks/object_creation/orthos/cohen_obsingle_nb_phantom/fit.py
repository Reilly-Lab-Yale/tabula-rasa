"""
Cohen obsingle NB fit -- single-zero reporter-informed expansion, NB-only.
Each U6 detection adds 1 phantom zero per (cell, CRE), not N barcodes.
"""
...
OUT_NAME = "cohen_obsingle_nb_phantom"
...
def main():
    ...
    with performance_report(filename=report_path):
        print("\nFitting by_cre (obsingle phantom NB)...", flush=True)
        t_cre = time.time()
        ortho_obj.fit_by_cre_models(client=client, dat=cohen,
                                    phantom_compress=True, nb_only=True,
                                    reporter_expansion="single")
        print(f"by_cre done in {time.time()-t_cre:.1f}s", flush=True)

        print("\nFitting by_cell_type (obsingle phantom NB)...", flush=True)
        t_ct = time.time()
    ...
    print(f"Saving to {DATA_DIR / OUT_NAME}...", flush=True)
    ortho_obj.save(str(DATA_DIR), OUT_NAME, client=client)
