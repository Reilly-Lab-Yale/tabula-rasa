#!/usr/bin/env python
"""
Takeshi obs NB ortho -- phantom-enabled, barcode-level oBC reporter.
"""
...
OUT_ROOT = DATA_DIR
OUT_NAME = "takeshi_obs_nb_phantom"
...
def main():
    ...
    with performance_report(filename=report_path):
        ...
        t_cre = time.time()
        ortho_obj.fit_by_cre_models(client=client, dat=takeshi, nb_only=True)
        print(f"by_cre done in {time.time()-t_cre:.1f}s", flush=True)

        print("\nFitting by_cell_type...", flush=True)
        t_ct = time.time()
        ortho_obj.fit_by_cell_type_models(client=client, dat=takeshi, nb_only=True)
