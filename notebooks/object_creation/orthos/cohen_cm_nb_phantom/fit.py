"""
Cohen CM NB fit -- full consider_missing, phantom compressed, NB-only.
...
def main():
    ...
    with performance_report(filename=report_path):
        print("\nFitting by_cre (CM phantom NB)...", flush=True)
        t_cre = time.time()
        ortho_obj.fit_by_cre_models(client=client, dat=cohen, nb_only=True)
        print(f"by_cre done in {time.time()-t_cre:.1f}s", flush=True)

        print("\nFitting by_cell_type (CM phantom NB)...", flush=True)
        t_ct = time.time()
        ortho_obj.fit_by_cell_type_models(client=client, dat=cohen, nb_only=True)
    ...
    ortho_obj.save(str(DATA_DIR), "cohen_cm_nb_phantom_20260401", client=client)
