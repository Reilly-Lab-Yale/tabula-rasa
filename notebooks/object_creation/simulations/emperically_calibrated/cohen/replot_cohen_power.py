"""Re-generate Cohen pairwise power plots from cached parquet.
...
power_df = pd.read_parquet(output_dir / "cohen_power_df.parquet")
power_grid = (
    power_df
...
for style_name, s in STYLES.items():
    ...
    for i, ct in enumerate(cell_types):
        ...
        ax.set_title(f"{ct_display}\n(n_cres={n_cres}, cells={cells_ct})", fontsize=s["title_fs"])
    ...
    svg_path = output_dir / f"cohen_pairwise_power_mwu_all_cell_types{s['suffix']}.svg"
    ...
    ax.set_title(f"Minimum detectable fold change — Cohen (MWU, α=0.05, "
                 f"{int(power_threshold*100)}% power)", fontsize=s["contour_title_fs"])
    ...
    svg_path2 = output_dir / f"cohen_pairwise_power_50pct_contours{s['suffix']}.svg"
