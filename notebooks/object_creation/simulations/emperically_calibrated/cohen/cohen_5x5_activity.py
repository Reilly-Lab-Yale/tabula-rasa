...
Scriptified from two_thirds_inactive.ipynb. Cohen-specific notes:
  ...
  - Cohen has "total transfection" -- all CREs present in all cell types,
    ...
    (all cell types see the same number of CREs).
  - 4 cell types: Mueller Glia, Interneuron, Bipolar, reference
...
def sim_name(gt_idx):
    return f"cohen_5x5_activity_gt{gt_idx}"
...
def phase_create(client):
    # Load canonical phantom ortho for real mu values.
    ...
    # Cohen has total transfection -- all cell types see all CREs
    combo_counts = (
    ...
    for gt_idx in range(N_GT_DRAWS):
        ...
        np.random.seed(gt_idx * 2000)  # distinct from shendure seeds
        ...
        libraries = [
            scm.simulate_library(
                ...
                reference_pooling=scm.COHEN_BOUNDS.n_negative_controls,
            )
...
def phase_metrics(client):
    ...
    combined.to_csv(outdir / "cohen_5x5_activity_summary.tsv", sep="\t", index=False)
    print(f"\nSaved to {outdir / 'cohen_5x5_activity_summary.tsv'}", flush=True)
